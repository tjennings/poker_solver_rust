...
Universal MP-lazy loading uses a private mmap-backed reader instead of eagerly
materializing owned payload vectors. The reader retains strict eager manifest,
header, SHA-256, CRC-64, structural, and probability-normalization validation;
row, action, probability, and semantic descriptors are materialized during a
query only as applicable. Queries use a compact sorted range locator rather than
a full row-key `HashMap`. Reader timings distinguish mapping, integrity checks,
validation, and index setup. Bundle files must remain immutable while loaded
because the reader relies on file-backed mappings. HU, eager MP, and legacy
