...
The mounted Tauri Game view now has a separate lazy-session adapter for two-player `universal_mp_lazy` bundles with a retained full MP config. It renders the preflop matrix and advances semantic lazy rows without reconstructing a HU tree or materializing the full MP tree. Eager MP, postflop bucket/card navigation, and N-player UI remain deferred.
