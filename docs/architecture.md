...
Universal MP-lazy loading keeps the existing strict payload checksum, CRC, decode,
and validation contract, but uses a compact sorted range locator for semantic
queries instead of constructing a full row-key `HashMap` at startup. This avoids
the large per-row index allocation without changing the on-disk format or the
public query key.
