...
- **Shared deterministic DCFR scheduler** across eager and lazy MP training: production runners own a monotonic `Instant` and feed completed meta-iterations plus elapsed `Duration` into pure scheduling state. Optional `training.dcfr_discount_interval_seconds` overrides the legacy iteration interval, arms when the scheduler starts at or beyond warmup or at the first later completed-batch observation that crosses warmup, and executes at most one pass per safe batch boundary. Missed wall-clock slots and crossed iteration boundaries are skipped without catch-up; wall-clock epochs count actual passes, while iteration mode retains the scheduled-boundary epoch. Optional `training.dcfr_discount_max_passes` separately caps successfully completed table sweeps: the Nth pass executes, the N+1 pass does not, and warmup or skipped boundaries do not consume the cap.
- **Runner-owned MP training-event routing**: eager and lazy runners may install the same thread-safe typed sink before training starts. DCFR schedule and completed-pass events route to that sink when present; otherwise core preserves detailed stderr logging for no-TUI and public API callers. `BlueprintTuiMetrics` implements the sink with latest-event-wins state, 60-second expiry for ordinary messages, and durable cap-reached state. The MP TUI renders that state on a clipped single-line row above the independent hotkey/snapshot footer, avoiding writes that corrupt the alternate screen.
...
When `dcfr_discount_max_passes` is reached, lazy post-discount subtree purge
also stops because purge is part of an actual discount pass. Ordinary
regret-threshold traversal pruning and negative-action traversal gating retain
their existing warmup and exploration behavior; the pass cap does not disable
either traversal-time mechanism.
...
includes training pauses, snapshots, and discount sweeps. The completed-pass
counter is likewise process-local and starts at zero for a newly created
scheduler, including one seeded with a nonzero meta-iteration. A new process
starts a new timer, so process downtime is not counted. Persisting the pass
epoch, remaining deadline, and completed-pass count requires an atomic MP resume
design and is deferred. A 600-second interval capped at 40 passes therefore
represents 400 scheduler minutes after warmup only within one uninterrupted
process; it is a pass cap, not a DCFR factor-epoch cap.
