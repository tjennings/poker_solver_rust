...
| `prune_after_iterations` | `0` | Warmup boundary before the opt-in MP negative-action subtree purge can start. Ordinary MP traversal pruning is disabled |
| `prune_threshold` | `-300` | Legacy ordinary traversal-pruning threshold. Currently ignored by MP training |
| `prune_explore_pct` | `0.05` | Legacy ordinary traversal-pruning exploration rate. Currently ignored by MP training |
