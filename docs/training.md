...
| `prune_threshold` | `-300` | Cumulative regret threshold for traversal pruning. In lazy sparse training, immediate terminal children are protected, and nonterminal branches below this threshold can only be skipped when their current regret-matched probability is zero |
