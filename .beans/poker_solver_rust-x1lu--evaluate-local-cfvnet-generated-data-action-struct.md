...
updated_at: 2026-05-04T15:10:53Z
---
...
Code inspection shows current river domain datagen parses nested bet_sizes, but build_turn_game_inner maps bet_sizes[0] to first bet sizes and flattens bet_sizes[1..] into one shared raise-size pool. For boundary_net_river_datagen.yaml this means first bets are 25/50/100%, while every raise depth uses 25/75%. Entries like "a" are skipped during parsing; in this path add_allin_threshold and force_allin_threshold are 0.0, so all-in only appears when a configured amount clamps to the full stack, not as a guaranteed standalone action. This is not Supremus's four-bucket schedule of first, second, third, remaining actions, each of which explicitly includes A.
