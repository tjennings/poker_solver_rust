// GPU kernel for sampling random river situations.
//
// Generates `batch_size` random river boards (5 unique cards each) and
...
// ------------------------------------------------------------------
// Phase 1: Sample 5 unique board cards per situation
// ------------------------------------------------------------------
extern "C" __global__ void sample_boards(
    unsigned int* boards,       // [batch_size * 5] output
    ...
    // Fisher-Yates partial shuffle to pick 5 unique cards from 0..52
    ...
    for (int pick = 0; pick < 5; pick++) {
        ...
        boards[tid * 5 + pick] = (unsigned int)deck[pick];
    }
}

// ------------------------------------------------------------------
// Phase 2: Build blocking-aware ranges (1.0 or 0.0 per combo)
// ------------------------------------------------------------------
extern "C" __global__ void build_ranges(
    float* ranges_oop,          // [batch_size * 1326] output
    float* ranges_ip,           // [batch_size * 1326] output
    const unsigned int* boards, // [batch_size * 5] input (from phase 1)
    ...
    // Check if either hole card collides with any board card
    const unsigned int* board = &boards[sit * 5];
    int blocked = 0;
    for (int i = 0; i < 5; i++) {
