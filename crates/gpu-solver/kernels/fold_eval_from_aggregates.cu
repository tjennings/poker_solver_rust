// O(1) per-hand fold CFV computation using precomputed aggregates.
//
// Uses inclusion-exclusion: for each traverser hand (c1, c2):
...
//
// The same_hand_reach correction handles the case where the opponent can hold
// the exact same two cards as the traverser — those are double-subtracted
// by the per-card terms and must be added back once.
//
// One thread per (fold_terminal, hand) pair.

extern "C" __global__ void fold_eval_from_aggregates(
    ...
    const float* fold_amount_win,          // [num_fold_terminals]
    const float* fold_amount_lose,         // [num_fold_terminals]
    const unsigned int* fold_player,       // [num_fold_terminals]
    const float* total_opp_reach,          // [num_fold_terminals]
    const float* per_card_reach,           // [num_fold_terminals * 52]
    ...
    float total = total_opp_reach[term_idx];
    ...
    unsigned int card_base = term_idx * 52;
    float blocked = 0.0f;
