...
mod tests {
    ...
    fn test_parse_pot_relative() {
        let opts = BetSizeOptions::try_from(("50%,100%", "60%")).unwrap();
        assert_eq!(opts.bet.len(), 2);
        assert_eq!(opts.raise.len(), 1);
    }

    #[test]
    fn test_parse_allin() {
        let opts = BetSizeOptions::try_from(("a", "a")).unwrap();
        assert!(matches!(opts.bet[0], BetSize::AllIn));
    }

    #[test]
    fn test_parse_geometric() {
        let opts = BetSizeOptions::try_from(("2e", "")).unwrap();
        assert!(matches!(opts.bet[0], BetSize::Geometric(2, _)));
    }

    #[test]
    fn test_parse_prev_bet_relative() {
        let opts = BetSizeOptions::try_from(("", "2.5x")).unwrap();
        assert!(matches!(opts.raise[0], BetSize::PrevBetRelative(_)));
    }
}
