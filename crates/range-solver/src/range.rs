...
static RANGE_REGEX: Lazy<Regex> = Lazy::new(|| {
    ...
    // INVARIANT: regex pattern is a compile-time constant
    .unwrap()
});

static TRIM_REGEX: Lazy<Regex> = Lazy::new(|| {
    // INVARIANT: regex pattern is a compile-time constant
...
mod tests {
    ...
    fn test_parse_range() {
        ...
        assert_eq!(range.get_weight_suited(12, 10), 0.0);
    }

    #[test]
    fn test_range_plus() {
        let range: Range = "TT+".parse().unwrap();
        assert_eq!(range.get_weight_pair(8), 1.0); // TT
        assert_eq!(range.get_weight_pair(12), 1.0); // AA
        assert_eq!(range.get_weight_pair(7), 0.0); // 99
    }

    #[test]
    fn test_range_dash() {
        let range: Range = "QQ-99".parse().unwrap();
        assert_eq!(range.get_weight_pair(10), 1.0); // QQ
        assert_eq!(range.get_weight_pair(7), 1.0); // 99
        assert_eq!(range.get_weight_pair(6), 0.0); // 88
    }

    #[test]
    fn test_range_weights() {
        let range: Range = "AA:0.5".parse().unwrap();
        assert!((range.get_weight_pair(12) - 0.5).abs() < 1e-6);
    }

    #[test]
    fn test_range_roundtrip() {
        let input = "AA,KK,QQ,AKs,AKo";
        let range: Range = input.parse().unwrap();
