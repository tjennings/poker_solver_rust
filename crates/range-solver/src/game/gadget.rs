//! Per-boundary CFR-D gadget (Option A2).
//!
//! Injects a 4-node gadget subtree at each cfvnet depth-boundary terminal,
//! giving each player an opt-out action whose terminal payoff is the
//! supplied static CFV. Post-injection ordinal scheme:
//! - Ordinals 0..N: original cfvnet boundaries (stable).
//! - Ordinals N..3N: gadget terminals (pre-populated with opt-out values).
...
mod tests {
    ...
    /// Build a river game (no boundaries) for basic tests.
    ...
    // ======================================================================
    // Per-boundary gadget tests (Option A2)
    // ======================================================================
