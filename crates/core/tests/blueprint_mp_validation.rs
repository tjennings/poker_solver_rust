...
use poker_solver_core::blueprint_mp::{Chips, MAX_PLAYERS, Seat};
...
fn tree_structure_reasonable() {
    ...
    let has_fold = tree.nodes.iter().any(|n| {
        matches!(
            n,
            MpGameNode::Terminal {
    ...
    let has_showdown = tree.nodes.iter().any(|n| {
        matches!(
            n,
            MpGameNode::Terminal {
...
fn training_reduces_exploitability_direction() {
    ...
    let br1 = compute_exploitability(&tree, &storage, 200, BUCKET_COUNTS, 0.0, Chips::ZERO);

    run_iterations(&tree, &storage, 500, 99);
    let br2 = compute_exploitability(&tree, &storage, 200, BUCKET_COUNTS, 0.0, Chips::ZERO);
...
fn three_player_convergence_smoke() {
    ...
    assert_eq!(
        result.meta_iterations, 200,
        "should complete 200 meta-iterations"
    );
}
...
fn three_player_convergence_smoke_strategy_nonzero() {
    ...
    assert!(
        any_nonzero,
        "3-player training should produce non-zero strategy sums"
    );
}
